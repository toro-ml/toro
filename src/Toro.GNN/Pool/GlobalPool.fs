...
module GlobalPool =
    ...
    let globalMeanPool (x: Tensor) (batch: Tensor) (numGraphs: int) : Result<Tensor, ToroError> =
    ...
    let globalSumPool (x: Tensor) (batch: Tensor) (numGraphs: int) : Result<Tensor, ToroError> =
        let features = x.Shape[1]
        ...
        result {
            ...
            let! batchIdx = batch.unsqueeze 1
            let! batchIdx = batchIdx.expand [ x.Shape[0]; features ]
            return! out.scatterAdd (0, batchIdx, x)
        }

    /// Max of node features per graph.
    let globalMaxPool (x: Tensor) (batch: Tensor) (numGraphs: int) : Result<Tensor, ToroError> =
        result {
            ...
            for i in 0 .. numGraphs - 1 do
                ...
                let! selected = x.indexSelect (0, Tensor.ofTorchTensor indices |> Result.defaultValue x)

...
/// use <c>scoped { }</c> for automatic disposal of intermediate tensors.
type Tensor internal (inner: torch.Tensor) =
    ...
    static member zeros(shape: int list, dtype: DType, device: Device) =
        Tensor(torch.zeros (Shape.toInt64Array shape, dtype = DType.toTorch dtype, device = Device.toTorch device))

    /// Create a tensor of ones with the given shape.
    static member ones(shape: int list, dtype: DType, device: Device) =
        Tensor(torch.ones (Shape.toInt64Array shape, dtype = DType.toTorch dtype, device = Device.toTorch device))
    ...
    static member rand(shape: int list, dtype: DType, device: Device) =
        Tensor(torch.rand (Shape.toInt64Array shape, dtype = DType.toTorch dtype, device = Device.toTorch device))

    /// Create a tensor with standard-normal random values.
    static member randn(shape: int list, dtype: DType, device: Device) =
        Tensor(torch.randn (Shape.toInt64Array shape, dtype = DType.toTorch dtype, device = Device.toTorch device))

    /// Create a 1-D tensor with values [0, stop).
    static member arange(stop: float, dtype: DType, device: Device) =
        Tensor(torch.arange (toScalar stop, dtype = DType.toTorch dtype, device = Device.toTorch device))

    /// Create a 1-D tensor with values [start, stop).
    static member arange(start: float, stop: float, dtype: DType, device: Device) =
        Tensor(torch.arange (toScalar start, toScalar stop, dtype = DType.toTorch dtype, device = Device.toTorch device))
    ...
    static member linspace(start: float, stop: float, steps: int, dtype: DType, device: Device) =
        Tensor(torch.linspace (start, stop, int64 steps, dtype = DType.toTorch dtype, device = Device.toTorch device))
    ...
    static member logspace(start: float, stop: float, steps: int, ?``base``: float, ?dtype: DType, ?device: Device) =
        ...
        Tensor(torch.logspace (start, stop, int64 steps, b, dtype = DType.toTorch dt, device = Device.toTorch dev))

    /// Create a 2-D identity matrix.
    static member eye(n: int, dtype: DType, device: Device) =
        Tensor(torch.eye (int64 n, dtype = DType.toTorch dtype, device = Device.toTorch device))
    ...
    static member randint(high: int64, shape: int list, dtype: DType, device: Device) =
        Tensor(torch.randint (high, Shape.toInt64Array shape, dtype = DType.toTorch dtype, device = Device.toTorch device))

    /// Create a random permutation of integers [0, n).
    static member randperm(n: int64, dtype: DType, device: Device) =
        Tensor(torch.randperm (n, dtype = DType.toTorch dtype, device = Device.toTorch device))
    ...
    static member ofTorchTensor(t: torch.Tensor) = Tensor(t)

    // --- Dimension query ---

    /// Return the size of dimension d.
    member _.dim(d: int) = inner.size d |> int

    // --- Arithmetic (tensor-tensor) ---

    /// Elementwise addition.
    member _.add(other: Tensor) = Tensor(inner.add other.Inner)

    /// Elementwise subtraction.
    member _.sub(other: Tensor) = Tensor(inner.sub other.Inner)

    /// Elementwise multiplication.
    member _.mul(other: Tensor) = Tensor(inner.mul other.Inner)

    /// Elementwise division.
    member _.div(other: Tensor) = Tensor(inner.div other.Inner)

    // --- Arithmetic (scalar) ---

    /// Add a scalar to each element.
    member _.addScalar(s: float) = Tensor(inner.add (toScalar s: Scalar))

    /// Multiply each element by a scalar.
    member _.mulScalar(s: float) = Tensor(inner.mul (toScalar s: Scalar))

    /// Subtract a scalar from each element.
    member _.subScalar(s: float) = Tensor(inner.sub (toScalar s: Scalar))

    /// Divide each element by a scalar.
    member _.divScalar(s: float) = Tensor(inner.div (toScalar s: Scalar))

    // --- Matrix ops ---

    /// Matrix multiplication.
    member _.matmul(other: Tensor) = Tensor(inner.matmul other.Inner)

    /// Transpose a 2-D tensor.
    member _.t() = Tensor(inner.t ())

    /// Swap two dimensions.
    member _.transpose(dim0: int, dim1: int) =
        Tensor(inner.transpose (int64 dim0, int64 dim1))

    // --- Shape manipulation ---

    /// Reshape to the given shape.
    member _.reshape(shape: int list) =
        Tensor(inner.reshape (Shape.toInt64Array shape))

    /// View with the given shape (must be contiguous).
    member _.view(shape: int list) =
        Tensor(inner.view (Shape.toInt64Array shape))

    /// Flatten dimensions [startDim, endDim].
    member _.flatten(startDim: int, endDim: int) =
        Tensor(inner.flatten (int64 startDim, int64 endDim))

    /// Flatten all dimensions to 1-D.
    member _.flattenAll() = Tensor(inner.flatten (0L, -1L))

    /// Remove a size-1 dimension.
    member _.squeeze(dim: int) = Tensor(inner.squeeze (int64 dim))

    /// Insert a size-1 dimension.
    member _.unsqueeze(dim: int) = Tensor(inner.unsqueeze (int64 dim))

    /// Return a contiguous copy.
    member _.contiguous() = Tensor(inner.contiguous ())

    /// Reorder dimensions.
    member _.permute(dims: int list) =
        Tensor(inner.permute (dims |> List.map int64 |> List.toArray))

    /// Reverse elements along the given dimensions.
    member _.flip(dims: int list) =
        Tensor(inner.flip (dims |> List.map int64 |> List.toArray))

    /// Broadcast to the given shape.
    member _.expand(shape: int list) =
        Tensor(inner.expand (Shape.toInt64Array shape))

    /// Repeat elements along a dimension.
    member _.repeatInterleave(repeats: int, dim: int) =
        Tensor(inner.repeat_interleave (int64 repeats, int64 dim))
    ...
    member _.norm(?ord: float, ?dim: int, ?keepDim: bool) =
        ...
        | Some d ->
            let kd = defaultArg keepDim false
            Tensor(inner.norm (d, keepdim = kd, p = p))
        | None -> Tensor(inner.norm (p = p))

    /// Cumulative sum along a dimension.
    member _.cumsum(dim: int) = Tensor(inner.cumsum (int64 dim))

    /// Cumulative product along a dimension.
    member _.cumprod(dim: int) = Tensor(inner.cumprod (int64 dim))

    /// $\log\sum e^{x_i}$ along a dimension, numerically stable.
    member _.logsumexp(dim: int, ?keepDim: bool) =
        let kd = defaultArg keepDim false
        Tensor(inner.logsumexp (int64 dim, kd))

    /// Count non-zero elements.
    member _.countNonzero(?dim: int) =
        match dim with
        | Some d -> Tensor(inner.count_nonzero ([| int64 d |]))
        | None -> Tensor(inner.count_nonzero ())

    /// Select elements from x or y by condition.
    static member where(condition: Tensor, x: Tensor, y: Tensor) =
        Tensor(torch.where (condition.Inner, x.Inner, y.Inner))

    // --- Unary ops ---

    /// $-x$
    member _.neg() = Tensor(inner.neg ())

    /// $\lvert x \rvert$
    member _.abs() = Tensor(inner.abs ())

    /// $\sqrt{x}$
    member _.sqrt() = Tensor(inner.sqrt ())

    /// $x^2$
    member _.sqr() = Tensor(inner.square ())

    /// $x^n$
    member _.pow(exponent: float) = Tensor(inner.pow (toScalar exponent))

    /// $e^x$
    member _.exp() = Tensor(inner.exp ())

    /// $\ln x$
    member _.log() = Tensor(inner.log ())

    /// $\log_2 x$
    member _.log2() = Tensor(inner.log2 ())

    /// $\log_{10} x$
    member _.log10() = Tensor(inner.log10 ())

    /// $\ln(1 + x)$
    member _.log1p() = Tensor(inner.log1p ())

    /// $2^x$
    member _.exp2() = Tensor(inner.exp2 ())

    /// $e^x - 1$
    member _.expm1() = Tensor(inner.expm1 ())

    /// $1/x$
    member _.reciprocal() = Tensor(inner.reciprocal ())

    /// $1/\sqrt{x}$
    member _.rsqrt() = Tensor(inner.rsqrt ())

    /// Sign of each element: -1, 0, or 1.
    member _.sign() = Tensor(inner.sign ())

    /// $\sin x$
    member _.sin() = Tensor(inner.sin ())

    /// $\cos x$
    member _.cos() = Tensor(inner.cos ())

    /// $\tan x$
    member _.tan() = Tensor(inner.tan ())

    /// $\arcsin x$
    member _.asin() = Tensor(inner.asin ())

    /// $\arccos x$
    member _.acos() = Tensor(inner.acos ())

    /// $\arctan x$
    member _.atan() = Tensor(inner.atan ())

    /// $\arctan(y/x)$ with correct quadrant.
    member _.atan2(other: Tensor) = Tensor(inner.atan2 other.Inner)

    /// $\lceil x \rceil$
    member _.ceil() = Tensor(inner.ceil ())

    /// $\lfloor x \rfloor$
    member _.floor() = Tensor(inner.floor ())

    /// Round to the nearest integer.
    member _.round() = Tensor(inner.round ())

    /// Truncate toward zero.
    member _.trunc() = Tensor(inner.trunc ())

    /// Fractional part: $x - \lfloor x \rfloor$.
    member _.frac() = Tensor(inner.frac ())

    /// Gauss error function.
    member _.erf() = Tensor(inner.erf ())

    /// Complementary error function.
    member _.erfc() = Tensor(inner.erfc ())

    /// Inverse error function.
    member _.erfinv() = Tensor(inner.erfinv ())

    /// $\max(0, x)$
    member _.relu() = Tensor(inner.relu ())

    /// $\text{GELU}(x) = x \cdot \Phi(x)$
    member _.gelu() = Tensor(torch.nn.functional.gelu inner)

    /// $\text{SiLU}(x) = x \cdot \sigma(x)$
    member _.silu() = Tensor(torch.nn.functional.silu inner)

    /// $\tanh(x)$
    member _.tanh() = Tensor(inner.tanh ())

    /// $\sigma(x) = 1 / (1+e^{-x})$
    member _.sigmoid() = Tensor(inner.sigmoid ())
    ...
    member _.clamp(min: float, max: float) =
        Tensor(inner.clamp (toScalar min, toScalar max))
    ...
    member _.indexSelect(dim: int, index: Tensor) =
        Tensor(inner.index_select (int64 dim, index.Inner))

    /// Gather elements along a dimension by index.
    member _.gather(dim: int, index: Tensor) =
        Tensor(inner.gather (int64 dim, index.Inner))

    /// Narrow a dimension to [start, start+length).
    member _.narrow(dim: int, start: int64, length: int64) =
        Tensor(inner.narrow (int64 dim, start, length))

    /// Scatter-add src into self along dim using index.
    member _.scatterAdd(dim: int, index: Tensor, src: Tensor) =
        Tensor(inner.scatter_add (int64 dim, index.Inner, src.Inner))

    /// Add src into self along dim at positions given by index.
    member _.indexAdd(dim: int, index: Tensor, src: Tensor) =
        Tensor(inner.index_add (int64 dim, index.Inner, src.Inner, toScalar 1.0))

    /// Split into chunks along a dimension.
    member _.chunk(chunks: int, dim: int) : Tensor array =
        inner.chunk (int64 chunks, int64 dim) |> Array.map Tensor

    /// Split into sections of given sizes along a dimension.
    member _.split(splitSizes: int list, dim: int) : Tensor array =
        inner.split (splitSizes |> List.map int64 |> List.toArray, int64 dim)
        |> Array.map Tensor

    /// Remove a dimension, returning an array of tensors.
    member _.unbind(dim: int) : Tensor array =
        inner.unbind (int64 dim) |> Array.map Tensor
    ...
    member _.argsort(dim: int, ?descending: bool) =
        let desc = defaultArg descending false
        Tensor(inner.argsort (int64 dim, desc))
    ...
    member _.diagonal(?offset: int, ?dim1: int, ?dim2: int) =
        ...
        Tensor(inner.diagonal (int64 o, int64 d1, int64 d2))

    /// Create coordinate grids from 1-D tensors.
    static member meshgrid(tensors: Tensor array) : Tensor array =
        let ts = tensors |> Array.map _.Inner
        torch.meshgrid ts |> Array.map Tensor

    // --- Linear algebra ---

    /// Matrix-matrix product $A \times B$.
    member _.mm(other: Tensor) = Tensor(inner.mm other.Inner)

    /// Batched matrix-matrix product.
    member _.bmm(other: Tensor) = Tensor(inner.bmm other.Inner)

    /// Matrix-vector product.
    member _.mv(vec: Tensor) = Tensor(inner.mv vec.Inner)

    /// Dot product of two 1-D tensors.
    member _.dot(other: Tensor) = Tensor(inner.dot other.Inner)

    /// Determinant.
    member _.det() = Tensor(torch.linalg.det inner)

    /// Matrix inverse.
    member _.inverse() = Tensor(torch.linalg.inv inner)

    /// Solve $Ax = b$; returns $x$.
    member _.solve(b: Tensor) =
        Tensor(torch.linalg.solve (inner, b.Inner))
    ...
    member _.cholesky() = Tensor(torch.linalg.cholesky inner)
    ...
    member _.linalgNorm(?ord: float, ?dim: int, ?keepDim: bool) =
        ...
        match dim, ord with
        | Some d, Some o -> Tensor(torch.linalg.norm (inner, o, dims = [| int64 d |], keepdim = kd))
        | Some d, None -> Tensor(torch.linalg.norm (inner, dims = [| int64 d |], keepdim = kd))
        | None, Some o -> Tensor(torch.linalg.norm (inner, o))
        | None, None -> Tensor(torch.linalg.norm inner)

    /// Trace of a matrix (sum of diagonal elements).
    member _.trace() = Tensor(inner.trace ())

    /// Outer product of two 1-D tensors.
    member _.outer(other: Tensor) = Tensor(inner.outer other.Inner)

    /// Matrix exponential.
    member _.matrixExp() = Tensor(torch.linalg.matrix_exp inner)

    // --- Type / Device conversion ---

    /// Move to a device.
    member _.toDevice(device: Device) =
        Tensor(inner.``to`` (Device.toTorch device))

    /// Cast to a data type.
    member _.toDType(dtype: DType) =
        Tensor(inner.``to`` (DType.toTorch dtype))
    ...
    member _.requiresGrad(?requiresGrad: bool) =
        ...
        Tensor(inner.requires_grad_ rg)

    /// Compute gradients by backpropagation.
    member _.backward() = inner.backward ()
    ...
    member _.detach() = Tensor(inner.detach ())
    ...
    member _.convTranspose1d
        ...
        let g = int64 (defaultArg groups 1)
        let b = bias |> Option.map _.Inner |> Option.defaultValue null
        Tensor(torch.nn.functional.conv_transpose1d (inner, weight.Inner, b, s, p, op, g, d))
    ...
    member _.layerNorm(normalizedShape: int list, ?weight: Tensor, ?bias: Tensor, ?eps: float) =
        ...
        Tensor(torch.nn.functional.layer_norm (inner, ns, w, b, e))
    ...
    member _.instanceNorm
        ...
        let rv = runningVar |> Option.map _.Inner |> Option.defaultValue null
        Tensor(torch.nn.functional.instance_norm (inner, rm, rv, w, b, uis, m, e))
    ...
    member _.avgPool1d(kernelSize: int, ?stride: int, ?padding: int) =
        let s = int64 (defaultArg stride kernelSize)
        let p = int64 (defaultArg padding 0)
        Tensor(torch.nn.functional.avg_pool1d (inner, int64 kernelSize, stride = s, padding = p))

    /// Adaptive 2-D average pooling to a fixed output size.
    member _.adaptiveAvgPool2d(outputSize: int) =
        Tensor(torch.nn.functional.adaptive_avg_pool2d (inner, int64 outputSize))

    /// Adaptive 1-D average pooling to a fixed output length.
    member _.adaptiveAvgPool1d(outputSize: int) =
        Tensor(torch.nn.functional.adaptive_avg_pool1d (inner, int64 outputSize))
    ...
    member _.maskedFill(mask: Tensor, value: float) =
        Tensor(inner.masked_fill (mask.Inner, toScalar value))
    ...
    member _.oneHot(numClasses: int) =
        Tensor(torch.nn.functional.one_hot(inner, int64 numClasses).``to`` torch.float32)

    // --- Misc ---

    /// Return a deep copy.
    member _.clone() = Tensor(inner.clone ())

    // --- Persistence ---

    /// Save to a file.
    member _.save(path: string) = inner.save path

    /// Load a tensor from a file.
    static member load(path: string) = Tensor(torch.Tensor.load path)

    // --- Scalar extraction ---

    /// Extract a float32 scalar.
    member _.toFloat32Scalar() = inner.ToSingle()

    /// Extract a float64 scalar.
    member _.toFloat64Scalar() = inner.ToDouble()

    /// Extract an int32 scalar.
    member _.toInt32Scalar() = inner.ToInt32()

    /// Extract an int64 scalar.
    member _.toInt64Scalar() = inner.ToInt64()

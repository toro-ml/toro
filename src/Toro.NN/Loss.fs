...
module private LossInternal =
    let inline wrap1 ([<InlineIfLambda>] f) (inp: Tensor) (target: Tensor) : Tensor =
        f (inp.Inner, target.Inner) |> Tensor.ofTorchTensor
...
module Loss =
    ...
    let smoothL1 (beta: float) (inp: Tensor) (target: Tensor) : Tensor =
        TorchSharp.torch.nn.functional.smooth_l1_loss (inp.Inner, target.Inner, beta = beta)
        |> Tensor.ofTorchTensor
    ...
    let huber (delta: float) (inp: Tensor) (target: Tensor) : Tensor =
        TorchSharp.torch.nn.functional.huber_loss (inp.Inner, target.Inner, delta = delta)
        |> Tensor.ofTorchTensor

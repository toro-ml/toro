...
let ``LSTM zeroState produces correct shapes`` () =
    let lstm = LSTM.initDefault 10 20 F32 Cpu
...
let ``LSTM step produces correct output shape`` () =
    ...
    let input = Tensor.randn ([ 2; 10 ], F32, Cpu)
...
let ``RNN.scan with LSTM processes full sequence`` () =
    let lstm = LSTM.initDefault 8 16 F32 Cpu
    let input = Tensor.randn ([ 2; 5; 8 ], F32, Cpu)
...
let ``GRU zeroState produces correct shape`` () =
    let gru = GRU.initDefault 10 20 F32 Cpu
...
let ``RNN.scan with GRU processes full sequence`` () =
    let gru = GRU.initDefault 8 16 F32 Cpu
    let input = Tensor.randn ([ 2; 5; 8 ], F32, Cpu)

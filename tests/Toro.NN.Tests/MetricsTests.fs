...
let ``accuracy of identical predictions is 1`` () =
    let pred = Tensor.ofList ([ 0L; 1L; 2L; 0L ], Cpu)
    let target = Tensor.ofList ([ 0L; 1L; 2L; 0L ], Cpu)
...
let ``accuracy of all wrong predictions is 0`` () =
    let pred = Tensor.ofList ([ 0L; 0L; 0L; 0L ], Cpu)
    let target = Tensor.ofList ([ 1L; 1L; 1L; 1L ], Cpu)
...
let ``accuracy of half correct is 0.5`` () =
    let pred = Tensor.ofList ([ 0L; 1L; 0L; 1L ], Cpu)
    let target = Tensor.ofList ([ 0L; 1L; 1L; 0L ], Cpu)
...
let ``accuracyFromLogits takes argmax`` () =
    let logits = Tensor.ofList ([ [ 2.0f; 0.1f; 0.1f ]; [ 0.1f; 2.0f; 0.1f ] ], Cpu)


    let target = Tensor.ofList ([ 0L; 1L ], Cpu)
...
let ``precision for perfect predictions`` () =
    let pred = Tensor.ofList ([ 0L; 1L; 1L; 0L ], Cpu)
    let target = Tensor.ofList ([ 0L; 1L; 1L; 0L ], Cpu)
...
let ``precision with false positives`` () =
    let pred = Tensor.ofList ([ 1L; 1L; 1L; 1L ], Cpu)
    let target = Tensor.ofList ([ 0L; 0L; 1L; 1L ], Cpu)

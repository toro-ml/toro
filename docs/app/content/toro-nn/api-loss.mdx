...
| `klDiv inp target` | $\text{KL}(p \| q)$ — log-probs input, probs target |
| `l1 inp target` | $\text{L1} = (1/n)\sum\lvert x_i - y_i \rvert$ |
| `mse inp target` | $\text{MSE} = (1/n)\sum(x_i - y_i)^2$ |
| `nll inp target` | $\text{NLL} = -(1/n)\sum x_{i,y_i}$ |
| `smoothL1 beta inp target` | Smooth L1 (Huber) loss with transition at beta |
...
- `beta` : `float` — transition point between L2 and L1
- `inp` : `Tensor`
- `target` : `Tensor`

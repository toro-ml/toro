...
Toro.NN provides a `LrSchedule` discriminated union and a `Scheduler` record that adjust the learning rate automatically.

`LrSchedule.lrAt` is a pure function that computes the learning rate for any step:

```fsharp
let lr = LrSchedule.lrAt baseLr (Exponential 0.9) step
```

| Schedule                          | Description                                       |
| --------------------------------- | ------------------------------------------------- |
| `StepDecay(stepSize, gamma)`      | Multiply LR by gamma every stepSize steps         |
| `Exponential gamma`               | Multiply LR by gamma each step                    |
| `CosineAnnealing(tMax, etaMin)`   | Cosine decay to etaMin over tMax steps, then reset|
| `LinearWarmup warmupSteps`        | Linear warmup from 0 to base LR, then hold        |
